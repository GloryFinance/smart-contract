...
contract GloryStakingManager is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        //   pending reward = (user.amount * pool.accGloryPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accGloryPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. Glory to distribute per block.
        uint256 lastRewardBlock; // Last block number that Glory distribution occurs.
        uint256 accGloryPerShare; // Accumulated Glory per share, times 1e12. See below.
    ...
    uint256 public MAX_STAKING_ALLOCATION = 105_000_000 * 10 ** 18; // 105M
    ...
    function pendingGlory(
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 gloryReward = multiplier
                ...
                .mul(pool.allocPoint)
                .div(totalAllocPoint);
            accGloryPerShare = accGloryPerShare.add(
                gloryReward.mul(1e12).div(lpSupply)
            );
        }
        uint256 pending = user.amount.mul(accGloryPerShare).div(1e12).sub(
            user.rewardDebt
        );
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accGloryPerShare = pool.accGloryPerShare.add(
            gloryReward.mul(1e12).div(lpSupply)
        );
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(
        ...
        user.rewardDebt = user.amount.mul(pool.accGloryPerShare).div(1e12);
    ...
    function payOrLockupPendingGlory(uint256 _pid) internal {
        ...
        uint256 pending = user.amount.mul(pool.accGloryPerShare).div(1e12).sub(
            user.rewardDebt
        );
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        if (
            address(gloryReferral) != address(0) && referralCommissionRate > 0
        ) {
