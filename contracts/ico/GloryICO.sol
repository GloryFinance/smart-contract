...
contract GloryICO is Initializable, ContextUpgradeable, OwnableUpgradeable {
    ...
    function setRoundWhiteListInfo(
        ...
        require(_startTimeRoundTwo < _endTimeRoundTwo, "invalid whitelist time");
    ...
    function _preCheckBuyRoundTwo(uint256 amount) internal view {
        require(amount > 0, "value must be greater than 0");
        require(block.timestamp >= startTimeRoundTwo, "ICO Round WhileList time does not start now");
        require(block.timestamp <= endTimeRoundTwo, "ICO Round whitelist  time is expired");
    }
